...
@Post({
  host: "http://some.api.google.com/fileupload-blob",
  contentType: "multipart/form-data",
})
class BlobFileFrame extends JinFrame {
  @Body()
  public declare readonly singleBlob: JinFile<Blob>;

  @Body()
  public declare readonly multiBlobs: JinFile<Blob>[];
}

@Put({
  host: "http://some.api.google.com/fileupload-put",
  contentType: "multipart/form-data",
})
...
@Patch({
  host: "http://some.api.google.com/fileupload-patch",
  contentType: "multipart/form-data",
})
        ...
        const fileValidations = await Promise.all(
          multipleFiles.map(async (file, index) => {
            ...
            if (!originalFileContent.equals(multipleFileBuffer)) {
              return {
                valid: false,
                error: `File content mismatch: ${file.name}`,
              };
            }
